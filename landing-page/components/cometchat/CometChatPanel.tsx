...
//     const time = new Date(sentAt * 1000).toLocaleTimeString();
//     const isMe = currentUid ? senderUid === currentUid : false;
//     return { id: String(msg.id), senderUid, text, time, isMe };
//   };
...
//   if (error) {
//     return (
//       <div className="w-full h-dvh flex items-center justify-center text-sm text-red-500">
//         {error}
//       </div>
//     );
//   }
...
//   if (!activeUid) {
//     return (
//       <div className="w-full h-dvh flex items-center justify-center text-center px-6">
//         <div>
//           <h2 className="text-lg font-bold text-[#111827] mb-1">{emptyTitle}</h2>
//           <p className="text-sm text-gray-500">{emptyHint}</p>
//         </div>
...
//   if (!ready || loadingThread) {
//     return (
//       <div className="w-full h-dvh flex items-center justify-center text-sm text-gray-500">
//         Loading chat...
//       </div>
//     );
//   }
...
//   return (
//     /*
//      * h-dvh — dynamic viewport height: shrinks when the iOS Safari keyboard opens,
//      * so the entire chat fits inside the visible area without any body scroll.
//      * overflow-hidden on this root prevents the page itself from scrolling.
//      */
//     <div className={className ?? "w-full h-dvh overflow-hidden"}>
//       <div className="flex flex-col h-full overflow-hidden">

//         {/* ── Header ─────────────────────────────────────────────────────────────
//             sticky top-0 instead of fixed: it scrolls with the flex column, so
//             it stays at the top of THIS container rather than the layout viewport.
//             On iOS Safari, fixed elements are anchored to the layout viewport which
//             doesn't shrink when the keyboard opens — sticky avoids that entirely.
//         */}
//         <div className="bg-white sticky top-0 z-20 px-4 py-3 w-full flex items-center justify-between shadow-sm border-b border-gray-100 flex-shrink-0">
//           <div className="flex items-center gap-3">
//             {onBack && (
//               <button
//                 onClick={() => {
...
//                     header.status === "online" ? "text-green-500" : "text-gray-400"
//                   }`}
//                 >
//                   {header.status === "online" ? "Online" : "Offline"}
//                 </span>
...
//         {/* ── Balance banners ─────────────────────────────────────────────────── */}
//         {billingEnabled && !canStart && (
//           <div className="px-4 py-3 bg-[#FFF7ED] border-b border-[#FED7AA] text-sm text-[#9A3412] flex items-center justify-between flex-shrink-0">
//             <span>Minimum balance ₹{minRequired} required to start chat.</span>
//             <button
//               onClick={billing?.onLowBalance}
//               className="text-[#FF7A1F] font-semibold hover:underline"
...
//         {billingEnabled && canStart && sessionStartedAt && timeExpired && (
//           <div className="px-4 py-3 bg-[#FEF2F2] border-b border-[#FECACA] text-sm text-[#B91C1C] flex-shrink-0">
//             Your chat time is up. Please top up to continue.
//           </div>
//         )}

//         {/* ── Message list ────────────────────────────────────────────────────────
//             flex-1 + min-h-0 + overflow-y-auto: this is the ONLY scrollable region.
//             No padding-bottom hack needed — the input bar is part of the flex column.
//         */}
//         <div
//           ref={scrollRef}
//           className="flex-1 min-h-0 overflow-y-auto p-4 flex flex-col space-y-4 bg-[#F8F9FA]"
//         >
//           {messages.map((msg) => (
//             <motion.div
//               initial={{ opacity: 0, y: 10 }}
...
//               transition={{ duration: 0.2 }}
//               key={msg.id}
//               className={`flex flex-col ${msg.isMe ? "items-end self-end" : "items-start self-start"} max-w-[85%] md:max-w-[70%]`}
//             >
//               <div
//                 className={`px-4 py-2.5 rounded-2xl ${
//                   msg.isMe
...
//         {/* ── Input bar ───────────────────────────────────────────────────────────
//             No longer fixed/absolute. It sits at the bottom of the flex column.
//             When the iOS keyboard opens, dvh shrinks the whole column so this bar
//             naturally rides up with the keyboard — no viewport tricks needed.
//             pb-safe uses env(safe-area-inset-bottom) for notch/home-bar padding.
//         */}
//         <div

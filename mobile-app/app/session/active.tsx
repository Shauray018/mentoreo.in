...
//       // Session went active — jump straight into the Sendbird chat.
//       // The chat screen's custom header owns the timer + end button from here.
//       if (s?.status === "active" && s.sendbird_channel_url) {
//         if (pollRef.current) clearInterval(pollRef.current);
//         router.replace(`/group-channel/${s.sendbird_channel_url}`);
//         return;
//       }

//       // Session is dead — go home
//       if (
//         !s ||
//         s.status === "completed" ||
//         s.status === "declined" ||
...
//       }
//     } catch (e) {
//       console.error("💥 [active session poll]", e);
//     } finally {
//       setLoading(false);
//     }
//   }, [user]);
...
//   useEffect(() => {
//     load();
//     // Poll every 3s while pending so the transition to chat feels snappy
...
//           setCancelling(true);
//           try {
//             await sessionsApi.cancel(session.id, user!.token);
//             if (pollRef.current) clearInterval(pollRef.current);
//             router.replace("/(tabs)");
//           } catch (e: any) {
...
//         {/* Cancel — students only */}
//         {user?.role === "student" && isPending && (
//           <TouchableOpacity
